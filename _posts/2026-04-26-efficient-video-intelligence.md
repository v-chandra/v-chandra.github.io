...
The first thing a video model does is encode each frame. Until recently, that meant picking an encoder family and accepting its weaknesses. Image-text contrastive models ([CLIP](https://arxiv.org/abs/2103.00020), [SigLIP](https://arxiv.org/abs/2303.15343), [SigLIP 2](https://arxiv.org/abs/2502.14786)) are the default VLM front-end for semantic retrieval but weak on dense prediction. Self-supervised [ViTs](https://arxiv.org/abs/2010.11929) ([DINOv2](https://arxiv.org/abs/2304.07193), [DINOv3](https://arxiv.org/abs/2508.10104)) excel on dense prediction (segmentation, depth, correspondence) because their training objective preserves fine-grained spatial structure, but their features are not aligned to language. Segmentation foundation models ([SAM](https://arxiv.org/abs/2304.02643), [SAM 2](https://arxiv.org/abs/2408.00714) and the compressed variants below) are specialists for object proposals and tracking. Dense-prediction specialists ([DepthAnything](https://arxiv.org/abs/2401.10891), [MiDaS](https://arxiv.org/abs/1907.01341), [DepthPro](https://arxiv.org/abs/2410.02073), [DepthLM](https://arxiv.org/abs/2509.25413)) handle depth.
...
1. Train a large proxy teacher by distilling from a diverse teacher pool: DINOv2 and DINOv3 (self-supervised dense features), the SAM family (SAM, SAM 2, [SAM 3](https://arxiv.org/abs/2511.16719)) for segmentation, and CLIP / SigLIP / SigLIP-SO400M for vision-language alignment.
...
**Sliding-window and sparse attention.** [LongLLaMA](https://arxiv.org/abs/2307.03170), [Mistral](https://arxiv.org/abs/2310.06825)'s sliding-window, and [DeepSeek's Native Sparse Attention](https://arxiv.org/abs/2502.11089). Each restricts attention to a local or learned subset of tokens.

**Linear attention.** [Performer](https://arxiv.org/abs/2009.14794), [Linformer](https://arxiv.org/abs/2006.04768), and [Nyströmformer](https://arxiv.org/abs/2102.03902) (Xiong et al., AAAI 2021), which uses Nyström-based low-rank approximation of the softmax kernel to achieve linear complexity. Recent production systems extend this thread: [Qwen3-Next](https://huggingface.co/Qwen/Qwen3-Next-80B-A3B-Instruct) pairs [Gated DeltaNet](https://arxiv.org/abs/2412.06464) (a linear-attention variant) with full attention in a 3:1 ratio. These approaches help when sequence length dominates compute.

**Hybrid architectures.** [Mamba](https://arxiv.org/abs/2312.00752)-Transformer hybrids ([Jamba](https://arxiv.org/abs/2403.19887), [Nvidia Nemotron Nano 2](https://arxiv.org/abs/2508.14444)) keep self-attention for short-range relationships and use SSM blocks for long-range dependencies. For video this maps naturally: most spatial reasoning is local, while temporal reasoning extends across many frames.

The structural pattern that holds for video is **factorized spatial-temporal attention**. Spatial attention within a frame is O(P²) where P is patches per frame and small; temporal attention across frames is O(T²) where T is frame count and can be large. Full attention on the spatial axis combined with linear or sparse attention on the temporal axis works well for most workloads, and recent open-weight video VLMs ([Qwen3-VL](https://arxiv.org/abs/2511.21631), [LLaVA-Video](https://arxiv.org/abs/2410.02713)) converge here.
...
**Encoder stitching** is the historical default: separate audio and visual encoders feed pooled embeddings into a language model. Cheap and modular, but cross-modal alignment is shallow because the encoders never see each other's data during training. **Native multimodal training** treats text, image, video, and audio tokens uniformly through a shared backbone. [Qwen3-Omni](https://arxiv.org/abs/2509.17765) is the strongest open-weight example as of April 2026, with state-of-the-art results on 22 of 36 audio and audio-visual benchmarks (32 of 36 among open-source models) while sharing weights with the visual stack, and Gemini's native multimodal architecture follows a similar internal pattern.

[EgoAVU](https://arxiv.org/abs/2602.06139) (CVPR 2026 Highlight) takes a third path. Rather than propose a new fusion architecture, EgoAVU builds the first large-scale egocentric audio-visual benchmark and dataset and evaluates how existing VLMs ([Qwen2-VL](https://arxiv.org/abs/2409.12191), Gemini, LLaMA 3) perform when audio embeddings are stitched alongside the visual tokens. Audio in egocentric video carries distinct information from third-person video: ambient sound, hand-object contact noise, the wearer's own voice, and conversational partners are all anchored on the wearer's body in ways they are not in YouTube-style footage. The evaluation shows that audio adds substantial signal on egocentric understanding tasks and that stitched audio encoders into existing VLMs are already a strong baseline; the headroom is in better data and training, not in radical architectural changes.
...
- **W4A16** (4-bit weights, 16-bit activations) is the default for VLMs and VLAs at the edge. Recent open releases including the Embedl-quantized [Cosmos-Reason2](https://huggingface.co/nvidia/Cosmos-Reason2-8B) (2B) variants show the recipe holds across multimodal architectures with minimal accuracy loss.
